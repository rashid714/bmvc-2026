...
# Your exact perfectly-sized taxonomies
...
def save_success_images():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"🖥️ Using device: {device}")

    # Create directories to save the GOOD images safely
    output_dir = project_root / "success_analysis"
    ...
    print("🧠 Loading Clean 9-Emotion Model Weights...")
    ...
    # Track how many PERFECT predictions we've saved per class (Max 5)
    MAX_SAVES = 5
    saved_counts = {
    ...
    print("📸 Hunting for PERFECT predictions... (Scanning Test Set)")
            ...
            for i, (true_val, pred_val) in enumerate(zip(int_lbls, int_preds)):
                if true_val == 1 and pred_val == 1 and i < len(INTENTIONS):
                    ...
                    if saved_counts["Intentions"][name] < MAX_SAVES:
                        filename = output_dir / "Intentions" / f"Correct_{sanitize(name)}_{batch_idx}.png"
                        torchvision.utils.save_image(images[0], filename, normalize=True)
                        saved_counts["Intentions"][name] += 1
                        total_saved += 1

            # 3. Check Action Successes (True Positives: Label is 1 AND Pred is 1)
            for i, (true_val, pred_val) in enumerate(zip(act_lbls, act_preds)):
                if true_val == 1 and pred_val == 1 and i < len(ACTIONS):
                    ...
                    if saved_counts["Actions"][name] < MAX_SAVES:
                        filename = output_dir / "Actions" / f"Correct_{sanitize(name)}_{batch_idx}.png"
                        torchvision.utils.save_image(images[0], filename, normalize=True)
                        saved_counts["Actions"][name] += 1
                        total_saved += 1

            # Stop if we hit 1500 batches so it doesn't run forever
            if batch_idx > 1500:
                break
                
            # Print a progress update every 100 batches
            if batch_idx % 100 == 0 and batch_idx > 0:
                print(f"   ... Scanned {batch_idx} batches. Captured {total_saved} perfect images so far.")

    print(f"\n✅ Done! Successfully extracted {total_saved} PERFECT prediction images.")
    print(f"📁 Check the '{output_dir}' folder to see the model's best moments!")
