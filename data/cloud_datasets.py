    ...
    def load_split(split: str = "train", limit: int = None) -> list[MultimodalSample]:
        try:
            ...
            for idx, row in df.iterrows():
                samples.append(MultimodalSample(
                    text=str(row['utterance']), emotion_label=4,
...
class MSCOCOCaptionsLoader:
    ...
    def load_split(split: str = "train", limit: int = None) -> list[MultimodalSample]:
        try:
            ...
            samples = []
            for item in dataset:
                samples.append(MultimodalSample(text=item.get("caption", ""), image_path=item.get("image_url") or item.get("image_id"), emotion_label=4, source_dataset="HF_COCO"))
            return samples
        except Exception as e: return []

# ==============================================================================
# 5. GOOGLE DRIVE MINE LOADER
# ==============================================================================
